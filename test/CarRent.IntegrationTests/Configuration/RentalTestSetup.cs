...
namespace CarRent.IntegrationTests.Configuration
{
    public static class RentalTestSetup
    {
        ...
        {
            await userRepository.InsertOrUpdate(x => x.Id == 1, new User { Id = 1, Name = "Aleksandar Trajkov" }, CancellationToken.None);
            ...
            await rentalRepository.InsertOrUpdate(x => x.Id == 1, new Rental { Id = 1, CarId = 1, UserId = 1, From = DateTimeOffset.Now.Date.AddHours(8), To = DateTimeOffset.Now.Date.AddHours(9) }, CancellationToken.None);
            await rentalRepository.InsertOrUpdate(x => x.Id == 2, new Rental { Id = 2, CarId = 4, UserId = 1, From = DateTimeOffset.Now.Date.AddHours(15), To = DateTimeOffset.Now.Date.AddHours(15).AddMinutes(45) }, CancellationToken.None);
            await rentalRepository.InsertOrUpdate(x => x.Id == 3, new Rental { Id = 3, CarId = 1, UserId = 1, From = DateTimeOffset.Now.Date.AddDays(5), To = DateTimeOffset.Now.Date.AddDays(5).AddHours(1) }, CancellationToken.None);
            await rentalRepository.InsertOrUpdate(x => x.Id == 4, new Rental { Id = 4, CarId = 1, UserId = 1, From = DateTimeOffset.Now.Date.AddDays(-5), To = DateTimeOffset.Now.Date.AddDays(-5).AddHours(1) }, CancellationToken.None);
        }
    }
}
