...
namespace CarRent.UnitTests.Application.UseCases.Cars.Handlers
{
    public class CreateNewCarTests
    {
        ...
        public CreateNewCarTests()
        {
            ...
            _carRepository.Setup(x => x.Insert(It.Is<Car>(x => x.UniqueId == _successData.UniqueId)))
                .ReturnsAsync(1);
            _carRepository.Setup(x => x.Insert(It.Is<Car>(x => x.UniqueId == _failData.UniqueId)))
                .ReturnsAsync(0);
        }
        ...
        public async Task WhenCallingGetAllCars_WithoutRepository_ExpectErrors()
        {
            ...
            Func<Task> result = async () => await new CreateNewCar(null!, _mapper).Handle(_successData, CancellationToken.None); ;
        ...
        public async Task WhenCallingGetAllCars_WithoutMapper_ExpectErrors()
        {
            // Arrange;
            var query = new GetAllCars.Query();
            Func<Task> result = async () => await new CreateNewCar(_carRepository.Object, null!).Handle(_failData, CancellationToken.None);
